...
def setup_params():
    ...
    opt = SimpleNamespace(
        ...
        save_dir='results/quantum_progressive',
        output_file=None,
...
def run_comparison_experiments():
    ...
    # 保存比较结果
    comparison_file = os.path.join(opt.save_dir, 'quantum_progressive_comparison.json')
...
def run_ablation_experiments():
    ...
    # 保存消融结果
    ablation_file = os.path.join(opt.save_dir, 'quantum_ablation_results.json')
    os.makedirs(opt.save_dir, exist_ok=True)
...
def calculate_incremental_contributions(results):
    ...
    # 保存贡献结果
    opt = setup_params()
    contrib_file = os.path.join(opt.save_dir, 'quantum_contributions.json')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='量子渐进式消融实验')
