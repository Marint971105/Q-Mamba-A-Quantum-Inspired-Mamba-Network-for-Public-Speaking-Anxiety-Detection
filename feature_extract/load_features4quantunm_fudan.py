...
def load_and_check_features(feature_dir, feature_file="features.pkl", sample_idx_to_check=None):
    ...
    Args:
        feature_dir: 特征文件所在的目录名称 (例如: "fudan_train_features")
...
if __name__ == "__main__":
    try:
        # # 检查训练集特征
        # print("检查复旦训练集特征...")
        # train_features = load_and_check_features("fudan_train_features", sample_idx_to_check=0)
        
        # # 检查验证集特征
        # print("\n检查复旦验证集特征...")
        # val_features = load_and_check_features("fudan_val_features", sample_idx_to_check=0)
        
        # 检查测试集特征
        print("\n检查复旦测试集特征...")
        test_features = load_and_check_features("fudan_test", sample_idx_to_check=0)
