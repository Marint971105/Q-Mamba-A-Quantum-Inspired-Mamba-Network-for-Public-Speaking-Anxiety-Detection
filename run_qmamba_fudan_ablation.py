...
def setup_params(weight_method='attention', sequence_model='mamba'):
    ...
    opt = SimpleNamespace(
        ...
        weight_method=weight_method,  # 权重计算方式
        num_layers=1,  # 层数
        ...
        save_dir='results/qmamba_fudan_ablation',
        output_file=None,
...
def run_experiment(weight_method, sequence_model='mamba'):
    ...
    """
    print(f"\n{'='*50}")
    print(f"开始Fudan实验: weight_method={weight_method}, sequence_model={sequence_model}")
    print(f"{'='*50}")
...
def run_ablation_study():
    """运行消融实验"""
    print("开始Fudan数据集QMamba权重计算方式消融实验")
    print("="*60)
    ...
    # 保存汇总结果
    summary_file = f"results/qmamba_fudan_ablation/summary_{int(time.time())}.json"
    os.makedirs("results/qmamba_fudan_ablation", exist_ok=True)
...
def run_single_comparison():
    ...
    # 保存比较结果
    comparison_file = f"results/qmamba_fudan_ablation/weight_comparison_{int(time.time())}.json"
    os.makedirs("results/qmamba_fudan_ablation", exist_ok=True)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Fudan数据集QMamba消融实验')
